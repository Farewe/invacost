...
author: "Leroy B, Kramer A, Courchamp C, Vaissière AC & Diagne C"
...
For more detailed information about invacost versions, see the file "NEWS" in
the invacost R package.
...
generally tend to focus on exchange rate since it is available for most countries
after 1960. 
...
invacost. **Note: we have progressively corrected the database such that they
should no longer occur from invacost v4.1**. If you find such 
...
the unreliable economic cost estimates. Furthermore, the database also includes 
...
two new fields in the database called `Probable_starting_year_adjusted` and 
...
currently underrepresented) and many places around the earth (most cost data are
...
increase seems to slow down/drop after 2013.
...
completeness when only looking at the number estimates. 
...
There are two main parameters to customize:
 ...
 analyses for the main paper we chose to start in 1970, because data for the
