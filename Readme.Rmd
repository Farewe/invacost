...
## How to distinguish between invacost v1 and invacost v2?
...
the starting date whatsoever, and also remove estimates for which it is not
possible to know if they occurred over a single or multiple years.
...
inadequate all models that predicted a decrease in costs after `r 2017 - quantiles["25%"] - 1`.
...
heteroscedasticity, temporal autocorrelation and outliers. Therefore, our choice of 
modelling methods was driven by methods relatively robust to such issues.
 ...
 covariance matrix estimations from the R package `sandwich`, and we 
 used the function `coeftest` from package `lmtest` to test for the significance
 ...
 function `lmrob`). We implemented
 robust regressions based on iterative weighted least squares since these type 
 of regressions are less sensitive to outliers than ordinary least square 
 regressions (and we do have outliers in INVACOST!). In addition, they provide
 error estimates which are robust to heteroskedasticity as well. *Thanks 
 Andrew Kramer for this addition*
 - **multiple adaptive regression splines** (Multiple Adaptive Regression 
 Splines, MARS, R package `earth`, function `earth`). This is a non-parametric 
 approach which will fit several linear splines, allowing to see non-linear 
 patterns in cost trends over time.
 ...
 time; however because of its sensitivity to heteroskedasticity, we used a
 location-scale method which consists in fitting two GAMs, one for the average
 trend and one for the variance.
...
does not come from statistical robustness but rather from the unknown missing
part of data.

It does not seem critical at this stage to have perfect statistics as long as 
...
Notwithstanding, we suggest to take time to carefully inspect your models.
Consider tweaking the number of parameters on GAM and MARS models, and look at
how the predicted shape of costs over time change (e.g. start from default 
parameters and progressively reduce it). 
