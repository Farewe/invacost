...
to biological invasions.
 ...
 - The first approach consists in calculating the *raw costs* that occurred over
 specific intervals of time. From this approach you can obtain the *observed*
 cumulated and 
 average annual costs of biological invasions for the chosen tim intervals.  
 This approach is generalisable to any subset of the invacost database,
 regardless of data quantity. However, it does account for the temporal dynamics
 of costs, and may thus underestimate the costs of invasions, especially for
 recent years for which we have incomplete data.
 ...
 of the quality of model fit, or of our *a priori* assumptions. Note that this
...
raw information is often difficult to compare because it can be based on
...
reliability descriptor has limitations. The most important one 
...
papers on the basis of thee lack of reproducibility of their methodologies.
...
documents whether the costs correspond to Observed or Potential costs. Choose
depending on your study objectives. In addition, costs can also be based on
direct observations or estimations, or can be based on extrpolations: this is 
...
over which costs occurred. Indeed, knowing the period over which a cost occurred
allows to derive cumulative costs, estimating average annual costs; it also 
...
`r nrow(invacost)` papers (`r round(nrow(invacost[which(is.na(invacost$Probable_starting_year) |
                      ...
                      % of papers), this information was not available.
...
then the impact was reported of one year only). These estimated stgarting and
...
To do this, we use the function we use the function `expandYearlyCosts`, to
which we provide the starting and ending year columns. It will store the
years over which economict costs occured in a column named `Impact_year`.
...
analyses; here, we will provide examples of how we suggest to do that.
...
calculating the observed cumulative and average costs over one specific period 
...
In the next lines of code we create a vector of weights for each year, which we
...
75% completeness treshold) would indicate an effect of the lack of data for 
...
The function called `costTrendOverTime` will fit all models automatically. If
we want, we can provide several parameters such as 
...
agains does ggplot2 stuff internally in invacost:
...
- `cost.data` contains total annual costs per year, upon which model are fitted
...
which parameters seems best for you.
...
There seven different regions in invacost, and sometimes cost are spread over 
different regions. Indeed, cost estimates in publications and reports often
correspond to data aggregated over several regions, several taxa at the same 
time, several types of cost etc. Most of the time, it is not possible to 
...
some of them together. For example, rats and mouses have often been analysed 
together, and we could decide to merge them in a single group:
...
for each taxon group, because the generic name "Diverse/Unspecified" in many 
different cases and for many different taxa. For example, we have cases where 
we know that the class is *mammals*, but the species is *Diverse/Unspecified*.
We do not want to merge these cases with e.g. *plants*. So we will create a 
...
winners are mosquitoes with a cumulted cost of US$ 158,622 millions for 1970-
...
users, please do not hesitate and we will implement it in the next versions of 
the package (of course I will add you as a contributor to the package when it
will be officially released).

# Citation

If you found the package and/or the tutorial useful, please do not hesitate to
cite the package (in addition to the data paper) as an acknowledgement for the
time spent in writing the package and this tutoriual. Like all R packages, 
